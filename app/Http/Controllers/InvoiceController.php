...
class InvoiceController extends Controller
{
    ...
    public function InvoicePendinglist(Request $request)
    {
        if ($request->ajax()) {
          $search = @$request->search;
          $perPage = @$request->perPage ? $request->perPage : 10;
          $data=Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','0');
          $datas = $data->paginate($perPage);
          return view('backend.components.invoice.table_pending',compact('datas'));
        }
        return view('backend.pages.invoice.invoice_pending');
    }
    ...
    public function InvoiceApprove($id){
        ...
        return view('backend.pages.invoice.invoice_approve',compact('invoice'));
    }// End Method
    ...
    public function DailyInvoiceReport(){
        return view('backend.pages.invoice.daily_invoice_report');
    }
    ...
    public function DailyInvoicePdf(Request $request){
         ...
         return view('backend.pages.pdf.daily_invoice_report_pdf',compact('allData','start_date','end_date'));
  }
