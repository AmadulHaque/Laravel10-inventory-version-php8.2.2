...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
  ...
  <div class="ms-auto">
    <div class="btn-group">
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addUnitrModal">Add Unit <i class="lni lni-plus"></i></button>
    </div>
  </div>
</div>
<!--end breadcrumb-->
...
<!-- add modal -->
@include( 'backend.components.unit.add')
<!-- Modal -->
<div class="modal fade" id="EditUnitModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form method="post" id="UnitUpdate">
				@csrf
...
<script>
  $(document).ready(function() {
   ...
   // table search perpage pagination setup end
	 ...
	 // insert row
	 $(document).on('submit', '#addUnitStore', function(e){
	      e.preventDefault();
				$('#addUnitrModal').modal('hide');
				$('.full_scren').removeClass('d-none');
	      let formData = new FormData($("#addUnitStore")[0]);
	      $.ajax({
	          url: "{{url('/unit/store')}}",
	          type: 'POST',
	          data: formData,
          ...
          $.ajax({
              ...
              success: function (data) {
                if (data.status==305) {
                   ...
                   Toast.fire({
                       icon: 'success',
                       title:"Unit Remove Successfully!"
                   })
               }
              }
          });
	 ...
	 // update row
	 $(document).on('submit', '#UnitUpdate', function(e){
	      e.preventDefault();
				$('#EditUnitModal').modal('hide');
				$('.full_scren').removeClass('d-none');
	      let formData = new FormData($("#UnitUpdate")[0]);
	      $.ajax({
	          url: "{{url('/unit/update')}}",
	          type: 'POST',
	          data: formData,
