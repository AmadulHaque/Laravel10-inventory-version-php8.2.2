...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
  <div class="breadcrumb-title pe-3">ExpenseType</div>
  <div class="ps-3">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb mb-0 p-0">
        <li class="breadcrumb-item"><a href="/dashboard"><i class="bx bx-home-alt"></i></a>
        </li>
        <li class="breadcrumb-item active" aria-current="page">All  ExpenseType</li>
      </ol>
    </nav>
  </div>
  <div class="ms-auto">
    <div class="btn-group">
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addExpenseTypeModal">Add ExpenseType <i class="lni lni-plus"></i></button>
    </div>
  </div>
</div>
<!--end breadcrumb-->
...
<!-- add modal -->
@include( 'backend.components.ExpenseType.add')
<!-- Modal -->
...
<script>
  $(document).ready(function() {
    ...
    function tableData(page=1){
      ...
      $.ajax({
           type: 'get',
           url: "{{ url('/expense-type') }}?page="+page,
           dataType: 'html',
           data: {
               search: search,
               perPage: perPage
	      ...
	      $.ajax({
              ...
              if(data.success == 1){
								...
								$('.full_scren').addClass('d-none');
									Toast.fire({
											icon: 'success',
											title:"ExpenseType Add Successfully!"
									})
          ...
          $.ajax({
              ...
              success: function (data) {
                if (data.status==305) {
                   ...
                   Toast.fire({
                       icon: 'success',
                       title:"ExpenseType Remove Successfully!"
                   })
                }
              }
          });
